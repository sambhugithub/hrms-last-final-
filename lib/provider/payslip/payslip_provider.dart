import 'package:flutter/cupertino.dart';
import 'package:hrms/model/employee/payslip/payslip_model.dart';
import 'package:hrms/repository/payslip/payslip_repository.dart';

import '../../helper/prefkeys.dart';
import '../../helper/string.dart';
import '../../view/widgets/shared_preference.dart';

class PayslipProvider extends ChangeNotifier{



  bool _isLoading = false;
  get isLoading => _isLoading;
  setIsLoading(value){
    _isLoading=value;
    notifyListeners();
  }

  String? _employeeid;
  get employeeid => _employeeid;
  setEmployeeId()async{
    _employeeid=await SharedPreference().getStringValue(PrefKeys.EMPLOYEEID);
    notifyListeners();
  }

  String? _organizationid;
  get organizationid => _organizationid;
  setOrganizationId()async{
    _organizationid=await SharedPreference().getStringValue(PrefKeys.ORGANIGATIONID);
    notifyListeners();
  }

  final String _currentYear=DateTime.now().year.toString();
  get currentYear => _currentYear;

  PayslipModel? _payslipList;
  PayslipModel? get payslipList => _payslipList;
  setPayslipList(v){
    _payslipList=v;
    notifyListeners();
  }



//get employee payslip list
  Future<Map<String, dynamic>> getEmployeePayslipList(context) async {
    await setEmployeeId();
    await setOrganizationId();
    _isLoading=true;
    int? flag;
    try {
      var parameter = {
        EMPLOYEE_ID : employeeid,
        ORGANIZATION_ID: organizationid
      };
      var result = await PayslipRepository.EmployeePayslipList(parameter: parameter);
      setIsLoading(false);
      flag = result['flag'];
      String errorMsg=result['message'];


      if (flag==1) {
        await setPayslipList(PayslipModel.fromJson(result['response']));
        //ToastMsg().showToast(Translation.of(context)!.translate(type=='login'?'clockin_successfull_msg':type=='logout'?'clockout_successfull_msg':'')!, Colors.green);
        //var getdata = result['data'][0];
        //type=='logout'?Routes.navigateToMainDashBoardAgain(context):null;
        return result;
      } else {
        await setPayslipList(PayslipModel.fromJson(result['response']));
        //ToastMsg().showToast(Translation.of(context)!.translate(type=='login'?'clockin_error_msg':type=='logout'?'clockout_error_msg':errorMsg)!, Colors.red);
        return {};
      }
    } catch (e) {
      return {};
    }
  }


}